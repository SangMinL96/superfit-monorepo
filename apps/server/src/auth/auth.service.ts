...
export class AuthService {
    ...
    async hpAuthNumberSend(params: { hp: string; randomNum: string }): Promise<ExecResultItf> {
        const result = await this.mysqlService.execQuery(phoneAuthNumQuery(), params);
    ...
    async hpAuthNumberCheck(params: { hp: string; authNum: string }) {
        const [result] = await this.mysqlService.getQuery<{ hp: string; auth_num }[]>(postPhoneAuthCheck(), params);
        console.log(params);
    ...
    async refreshTokenUser(params: { ent_code: string; name: string }) {
        const [result] = await this.mysqlService.getQuery<{ refresh_token: string }[]>(getRefreshTokenQuery(), params);
        return result;
    }
}
