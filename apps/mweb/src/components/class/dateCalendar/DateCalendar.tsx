...
function DateCalendar() {
    ...
    const [선택날짜, set선택날짜] = useState<string[]>([])
    ...
    const onClickDay = (date: Date) => {
        set선택날짜(prev => {
            if (prev.includes(dayjs(date).format("YYYY-MM-DD"))) {
                return prev.filter(f => f !== dayjs(date).format("YYYY-MM-DD"))
            }
            return prev.concat(dayjs(date).format("YYYY-MM-DD"))
        })
    }
    const renderTileContent = (e: TileArgs) => {
        if (선택날짜.includes(dayjs(e.date).format("YYYY-MM-DD"))) {
            return <div className={cx("tile_select")}>
                {checkSvg}
            </div>
        }
        return null
    ...
    return (
        <>
            <div className={cx(calendarStyles.fc_calencar)}>
                <DetailYearMonthPicker value={value} onChange={(e: any) => {
                    onViewChange(e);
                    onChange(dayjs(e.activeStartDate).format('YYYY-MM-DD'))
                }} />
                <Calendar
                ...
                <Wrap padding='0px 20px' margin='10px 0 0'>
                    <Button type='button' size={36} color='grey' name='date_˝list' onClick={() => setView일정(true)}>선택한 일정 모아보기</Button>
                </Wrap>
            </div>
            {view일정 && <BottomSheet onClose={() => setView일정(false)} open={view일정} title='선택한 일정'>
                            ...
                            <div key={month} className={cx(styles.month_group)}>
                                ...
                                <ul className={cx(styles.day_box)}>
                                    {days.sort((a, b) => {
                                        ...
                                        return <span key={item} className={cx(styles.day)}>{item}</span>
                                    })}
                                </ul>
                            </div>
                        )
